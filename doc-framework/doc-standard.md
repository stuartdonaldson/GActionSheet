...
A workspace is a repository containing two or more semi-independent projects, each with their
own deployment target, lifecycle, or ownership boundary. The workspace itself is not a
project — it is a container and coordination layer.

### When to Apply

The decision criterion is **lifecycle independence**, not project size. Apply workspace
structure when two or more components have independent release cycles, independent deployment
targets, or could have separate ownership. A large project organized into internal modules
does not qualify — those modules co-deploy and have no independent lifecycle.

Common signals:
- Each component has its own `.clasp.json`, `pyproject.toml`, `package.json`, or git submodule registration
- Components can be versioned and released independently
- A plugin or extension can be updated without releasing the core
- Components are or could be owned by different people or teams

### Workspace vs. Extended Tier

Extended tier describes a single project of high complexity (multiple stakeholder groups,
compliance requirements, distributed behavior). Workspace describes a repository containing
multiple projects. These are orthogonal: a workspace can contain an Extended-tier core
project alongside smaller sub-projects. Project size or complexity does not determine whether
to use workspace structure — lifecycle independence does.

### Plugin/Extension Pattern

A core application with independently-lifecycled plugins is a workspace. The core is a
sub-project at whatever tier its complexity warrants (often Extended). Each plugin is a
separate sub-project at Minimal or Standard. A plugin SDK consumed by plugin authors is a
library sub-project. Example:

```
workspace root
├── core/              ← Extended tier (large, complex, multi-stakeholder)
├── plugin-auth/       ← Standard tier (own release cycle, own deployment)
├── plugin-reporting/  ← Minimal tier (simpler, own lifecycle)
└── plugin-sdk/        ← library type (consumed by plugin authors)
```
...
|------|----------|---------|
| `CLAUDE.md` | Required | Workspace context: sub-project table, dependency chain, navigation |
| `CONTENTS.md` | Required | Project index — see `templates/CONTENTS.md` |
| `README.md` | Required | User-facing overview with links to sub-projects |
| `docs/CONTEXT.md` | Optional | Workspace-level shared context (goals, stakeholders) — only if meaningful content exists beyond what's in individual sub-project docs |
| `docs/DESIGN.md` | Optional | Cross-cutting architecture shared by multiple sub-projects |
| `knowledge-base/adr/` | Optional | ADRs for decisions that span multiple sub-projects or govern the workspace structure |
| `ROUTING.md` | Optional | Agent session routing guide — see `templates/ROUTING.md` |
| `tests/` | Optional | Integration tests that span multiple sub-projects — listed as a shared resource in CONTENTS.md |

### Sub-Project Ownership

Each sub-project owns its own `CLAUDE.md`, docs, ADRs, and unit/functional tests. The
workspace does not duplicate sub-project content. Shared canonical resources (specs, schemas,
test fixtures) and cross-project integration tests live at workspace root and are referenced
by sub-projects — never copied into them.

### Tier Assignment

Sub-projects are assigned tiers independently. The workspace itself has no tier — it uses
workspace structure regardless of the tier of its constituent projects.

### bd Scope Rule

- **Workspace bd** (root `.beads/`): cross-cutting concerns, initiatives spanning multiple sub-projects, workspace-structure ADRs.
- **Sub-project bd**: work scoped entirely within one sub-project.
- If a work item touches only one sub-project, it belongs in that sub-project's bd.
- Not all sub-projects need bd. Library or sample sub-projects with no active development may have no bd.
...
**Workspace files** (`CONTENTS.md`, `ROUTING.md`) follow workspace structure rules, not
project-tier rules. They are governed by the §Workspace section above, not by
Minimal/Standard/Extended placement rules.

### Directory Structure
| Directory | Purpose | Contents |
|-----------|---------|----------|
| `/doc-framework/` | Read-only copies of framework files | doc-standard.md, README.md, templates/, and optionally planning-guide.md. |
| `/adr/` (Minimal) or `/knowledge-base/adr/` (Standard/Extended) | Architecture decisions | Immutable once Accepted. See ADR Format in these standards. |
| `/docs/assets/` | Binary reference files | PDFs, images, diagrams. Create only when needed. |
| `/knowledge-base/lessons-learned/` | Staged LL incident files | Transient — one file per incident; deleted after batch resolve. Empty directory = no open incidents. |
| `/docs/interfaces/` | Protocol and API reference scoped to implementation | Create only when needed. |
| `/knowledge-base/references/` | AI-readable summaries of external documents | Create only when needed. See Reference Document Handling in these standards. |
...
that level yet — not that the funnel is inactive. Governed by `doc-framework/planning-guide.md`,
...
|----------|---------|-----------|
| `knowledge-base/VISION.md` | Strategic themes — years-horizon goals that initiatives must align with | Permanent; reviewed annually; rarely changes |
| `knowledge-base/ROADMAP.md` | Pre-beads funnel: §Funnel (one-liner ideas) and §Review (lean business cases) | Permanent; evolves at monthly/biweekly planning reviews |
| `knowledge-base/staging/<n>.md` | Active scoping for work not yet decomposable into beads issues | **Transient — 2-week max**; deleted when work enters beads or pushed back to §Review |
...
`doc-framework/templates/` in the central tooling location — use them when creating new documents.
...
- Standard / Extended: `/knowledge-base/adr/NNNN-short-decision-name.md`
...
| Staging document decomposed into beads | Delete `knowledge-base/staging/<n>.md` |
...
| External document updated | Update reference summary in /knowledge-base/references/ |
...
| Failure, gate finding, or rework occurs | Create `knowledge-base/lessons-learned/<YYYY-MM-DD>-<slug>.md`; do not resolve inline |
| Project phase changes (Standard/Extended, README has Status line) | Update README.md Status line to current phase |
| Session starts and `knowledge-base/lessons-learned/` has files | Prompt user to resolve staged LL incidents before beginning work |
| Gate or phase transition completes | Check `knowledge-base/lessons-learned/`; prompt for batch resolve if files present |
...
| New initiative identified (planning funnel in use) | Add one-liner to `knowledge-base/ROADMAP.md §Funnel` |
| Initiative passes value/risk review | Promote to `knowledge-base/ROADMAP.md §Review` with lean business case |
| Initiative selected for execution | Create `knowledge-base/staging/<n>.md`; identify open questions as spikes |
| Spike resolves open question | `bd close <spike-id> --reason="..."` + `bd remember "insight"` |
| Staging work fully decomposed into beads | Delete `knowledge-base/staging/<n>.md`; execute via `bd ready` |
| Staging document exceeds 2-week TTL | Push back to `ROADMAP.md §Review`; delete staging doc |
| Strategic theme changes | Update `knowledge-base/VISION.md`; review ROADMAP.md for alignment |
...
2. Create a summary in `/knowledge-base/references/` covering only sections relevant to this project
...
| knowledge-base/VISION.md | — | ≤400 | ≤400 | Optional (planning funnel); strategic themes only — stable, concise |
| knowledge-base/ROADMAP.md | — | ≤800 | ≤1200 | Optional (planning funnel); exceeding signals need for triage or promotion |
| knowledge-base/staging/<n>.md | — | ≤300 | ≤300 | Optional (planning funnel); transient, 2-week max; growing = needs more spikes first |
| knowledge-base/lessons-learned/<n>.md | ≤150 | ≤150 | ≤150 | Transient; observation + 5-why chain only; no candidates table; growing = resolve more frequently |
...
## Planning Funnel (Standard when bd is in use — doc-framework/planning-guide.md)

The planning funnel governs how ideas mature into executable beads issues. It is defined and
governed by `doc-framework/planning-guide.md`. This framework defines placement and size
...
CLAUDE.md structure is defined by `doc-framework/templates/CLAUDE.md` in the central tooling
