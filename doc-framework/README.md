...
Canonical repository location: `doc-framework/`.
Previous repository location: `docs/framework/`.
...
Copy the framework folder into the target project's `/doc-framework/`. Create document
...
2. Framework files present in `/doc-framework/` — note any absent or unexpected files
...
If `/doc-framework/README.md` is absent, the prior upgrade was incomplete or the framework
...
Copy all framework files from this folder into the target project's `/doc-framework/`,
...
`/doc-framework/templates/`. Apply the following rules:
