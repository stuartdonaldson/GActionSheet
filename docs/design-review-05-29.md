...
| L3 | Low — ❌ **REVERTED 2026-05-30** (correction) | Dead code | Every proxy call sends `Authorization: Bearer <oauthToken>`, but `doPost` authenticates on the shared `secret` only and never reads the header. | `SyncManager.js:511`, `WebApp.js:56-59` | **ADR-001 §Tradeoffs** ("Bearer tokens not propagated by Apps Script runtime") explains *why* the header is useless **to `doPost`** — but this misses a second auth layer. `UrlFetchApp` does not carry the caller's Google session; the Bearer token is required to pass GAS's own HTTP auth gate before the request reaches `doPost` at all. Removing it causes HTTP 401 on both `/dev` and versioned `/exec` (`access: ANYONE`) deployments. Confirmed by `sync.error: HTTP 401` in production logs after the removal. See `knowledge-base/references/workspace-addon-setup.md §Authentication` for the corrected two-layer model. | ❌ Removal was incorrect — Bearer headers restored to `_syncActionRows` and `_markDocNotFound`. `appsscript.json` reverted to `ANYONE_ANONYMOUS` (correct for `/exec` deployments — allows Node.js test infra to POST without OAuth; Bearer token in GAS callers handles auth for `/dev` and is harmless on anonymous endpoints). |
...
3. ~~**Correctness fixes**~~ ✓ **Done 2026-05-29** (set 3): C1 rename (`namedRangeId`→`globalId`
   ...
   Twin `[TST]` tickets created: gts-45k (M1), gts-ckj (M2), gts-dm7 (M3),
   gts-wpe1 (M4). TestFixtures.js `_TF_RESULT` now returns `globalId` field — Python
...
4. ~~**Risk/maintainability**~~ ✓ **Partially done 2026-05-29** (set 4): M5 (trailing
   ...
   column count + superseded-requirement refs); L3 (Bearer header removal — ❌ reverted 2026-05-30, see L3 correction);
