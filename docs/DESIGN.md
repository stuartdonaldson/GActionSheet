...
| VerifySync | Reads floating actions from the doc, reads ActionSheet rows for the same doc via a non-mutating Web App call, parses the in-doc tracker table when present, and reports progress plus mismatches; a floating action without an explicit trailing `(Status)` token is itself a verification failure. A paragraph that starts a token `(ACT\|AI)-\d+` but never completes the grammar (e.g. missing the colon) is reported as `unparseable-action-paragraph` (body index + leading text) rather than silently dropped — ADR-0027 rule 6, gts-xvlu |
