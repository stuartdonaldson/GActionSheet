...
> **Note:** the current UI uses a single Import button for both *browsing* and *importing*. We're evaluating splitting this into a clearer "view old/forwardable actions" step plus a separate, explicit "import" step — tracked as gts-csbv.3 under the UX Improvements epic. Documented here as it currently behaves.
