...
| `test.u2.json` | Restricted end user — single team | Reader on one team folder only (e.g. `testTeamAChild`). Used via `PROBE_AUTH_STATE=.auth/test.u2.json` / `pnpm run probe:test.u2`. |
...
or via pnpm: `pnpm run auth:test.u1`, `auth:test.u2`, `auth:test.u3`, `auth:nuuts.service`.
